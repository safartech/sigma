    ...
    <div class="container">
        ...
        <div class="row text-center">
            <div class="">
                ...
                <table class="col-lg-12 table-bordered table-condensed text-center" style="left: auto; right: auto; position: relative; width: 100%">
                    <thead>
                    <tr>
                    ...
                    @foreach($obligatoires as $matiere)
                        ...
                        <tr>
                            ...
                            {{--<td></td>--}}
                            <td class="text-center">{{ $matiere->appreciations[0]->appreciation ?? "" }}</td>
                ...
                @endphp
                <table class="col-lg-12 table-bordered table-condensed " style="left: auto; right: auto; position: relative; width: 100%">
                    <thead>
                    <tr>
                        <th class="text-center" style="background-color: cornflowerblue" >Moyenne
                            <br>du trimestre</th>
                        <th class="text-center" style="background-color: cornflowerblue">Rang</th>
                        <th class="text-center">Moy. forte</th>
                        <th class="text-center">Moy. faible</th>
                        <th class="text-center" style="">Moy. de classe</th>
                        @if($session->id==1)
                        @elseif($session->id==2)
                            <th class="text-center" colspan="2" >1er Trim</th>
                        @elseif($session->id==3)
                            <th class="text-center" colspan="2">1er Trim</th>
                            <th class="text-center" colspan="2" >2e Trim</th>
                            <th class="text-center" style="background-color: #ff595f">Moy. annuelle</th>
                        @endif
                        <th class=" text-center grey" >Absences</th>
                        <th class=" text-center grey" >Retards</th>
                        <th class=" text-center grey" ><b>ASSIDUITE</b></th>
                        <th class=" text-center grey" ><b>CONDUITE</b></th>
                        <th class=" text-center grey" ><b>TRAVAIL</b></th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <td class="text-center" style="background-color: cornflowerblue"><b>{{ $moyenne }}</b></td>
                        <td class="text-center" style="background-color: cornflowerblue"><b>{{ getRang($moys_tab,$moyenne) }} @if(is_numeric(getRang($moys_tab,$moyenne))) @if(getRang($moys_tab,$moyenne)==1) er @else e @endif @endif</b></td>
                        <td class="text-center">{{ $biggest_moy }}</td>
                        <td class="text-center">{{ $smallest_moy }}</td>
                        <td class="text-center">{{ $moy_of_classe }} </td>
                        @if($session->id==1)
                        @elseif($session->id==2)
                            <td class="text-center">{{ $s1_moyenne }}</td>
                            <td class="text-center"  ><span style="padding: 0px 10px">{{ $s1_rang }} @if($s1_rang==1) er @else e @endif</span></td>
                        @elseif($session->id==3)
                            <td class="text-center">{{ $s1_moyenne }}</td>
                            <td class="text-center"  ><span style="padding: 0px 10px">{{ $s1_rang }} @if($s1_rang==1) er @else e @endif</span></td>
                            <td class="text-center">{{ $s2_moyenne }}</td>
                            <td class="text-center"  ><span  style="padding: 0px 10px">{{ $s2_rang }} @if($s2_rang==1) er @else e @endif</span></td>
                            <td class="text-center" style="background-color: #ff595f">{{ number_format(($s1_moyenne+$s2_moyenne+$moyenne)/3,2) }}</td>
                        @endif
                        <td class="text-center">{{ $conseil->absences ?? "" }}</td>
                        <td class="text-center">{{ $conseil->retards ?? "" }}</td>
                        <td class="text-center">{{ $conseil->assiduite ?? "" }}</td>
                        <td class="text-center">{{ $conseil->conduite ?? "" }}</td>
                        <td class="text-center">{{ $conseil->travail ?? "" }}</td>
                    </tr>
                    </tbody>
                    {{--<tr class="">
                ...
                <div class="col-lg-12">
                    <table class="table-condensed table" style="border: 0px ; padding-bottom: 0px; padding-top: 0px">
                        <thead style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">
                        <tr style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">
                            <th style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">
                                <h5><b>Appréciations du Conseil de Classe : </b></h5>
                                <h5 class=""> {{ $conseil->ag ?? ""}} </h5>
                                {{--<h5 style="visibility: hidden" style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">0000</h5>--}}
                            </th>
                        ...
                        <tr>
                            ...
                            <th style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">
                                <h5 style="visibility: hidden" style="border: 0px   ; padding-bottom: 0px; padding-top: 0px">0000</h5>
                                <h5 ><b>Yao Mawupenkor TSATSU</b></h5>
                            </th>
                        </tr>
                        </thead >
                    </table>
                    {{--<div class="col-lg-6 ">
                        <div class="row pull-left" style="max-width: 50%">
                            <h5 class="pull-left"><b>Appréciations du Conseil de Classe : </b></h5>
                            <br>
                            <h5 class="pull-left" style="max-width: 50%; "><b>Elisabeth est très motivée mais son travail est en baisse. Elle peut mieux faire {{ $conseil->ag ?? ""}} </b></h5>
                            --}}{{--<br>--}}{{--
                        </div>
                        <br>
                    </div>
