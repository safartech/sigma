...
let bulletins = {
    ...
    methods:{
        saveMoys(){
            // alert(this.selectedClasseId)
            $('#loader').modal('show')
            instance.get('save_gen_moys_of_classe/'+this.selectedClasseId+'/'+this.selectedSessionId).then(res=>{
        ...
        loadDatas(){
            instance.get('load_bulletins_datas_from_admin').then(res=>{
        ...
        loadBulletin(){
            ...
            $('#loader').modal('show')
            console.log("session",this.selectedSession.id)
