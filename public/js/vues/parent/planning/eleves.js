...
//            defaultDate: '2018-05-12',
    ...
    methods:{
        hooveredTd(jour,h){
            ...
            if(c.length>0){
                if (this.hooveredCoursId == c[0].id){
                    return "primary"
                }else return ""
            }
        },
        ...
        getParsedCours(j){
            ...
            if(this.selectedEleveId){
                ...
                cours = j.cours.filter(x=>x.classe_id==this.selectedEleveId)
...
/*
        ...
        loadDatas(){
            instance.get('load_planning_for_students_from_parent').then(resp=>{
                ...
                this.responsables= resp.data.responsable
                this.eleves = resp.data.responsable[0].eleves
        ...
        getCoursMatiereColor(jour,h){
            ...
            if(c.length>0)
             //console.log(c[0].matiere.couleur3)
                return c[0].matiere.couleur3
            return ""
        },
        onEleveChange(eleveId){   //must

            this.selectedEleveId = eleveId

            this.selectedEleve = this.eleves.find(it=>it.id==eleveId)

            this.cours = this.selectedEleve.classe.cours
